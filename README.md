...
- **🔬 数据查询** — 远程查询 XRD、电镜、光谱数据，返回分析结果。
- **🔒 数据不出域** — 原始数据留在本地，别人提交分析请求，拿到的是报告和图表。
- **⚡ 计算调度** — 提交计算任务，自动分发到已注册的 HPC/GPU 节点。
- **🏢 跨组协作** — 课题组 A 的模拟工具、B 的 ML 模型、C 的数据管道，变成统一网络中的服务。
...
./target/release/agent-core register --token <registration_token> --name lab-sem-node
...
如果你的 Agent 没有 OpenAaaS 插件，访问 <https://api.open-aaas.com>
