...
传统云端方案要求数据离开本地：TB 级数据集必须上传，敏感样本交给第三方，实验室防火墙被迫开放入站端口。OpenAaaS 反其道而行——将 Agent 执行节点直接部署在数据本地，网络只传输任务描述和结果，原始数据原地处理。

| | 传统云端方案 | OpenAaaS 近数据端方案 |
|---|---|---|
| 数据流向 | 本地 → 云端 → 本地 | **原地不动** |
| 网络传输 | 原始数据（TB 级） | 仅任务描述（KB 级）+ 结果 |
| 防火墙要求 | 需开放入站端口 | **仅出站 HTTP 即可** |
| 敏感数据 | 必须出域 | **不出实验室** |
| 延迟 | 受带宽限制 | 本地计算，极低延迟 |

Server 只是一个轻量索引层（注册、路由、心跳），真正的数据和能力都在本地节点。
...
```
  ...
  客户端 Agent  ──────────────────────>  OpenAaaS Server
  (pi / Claude / Kimi)                  （轻量索引层）
                                        注册 · 路由 · 心跳
  ▲                                     不存储 · 不中转数据
  │
  └──────────────────────────────────────────────────────────
...
| 节点自组网 | 节点主动向网络注册并轮询任务，Server 仅维护索引，不存储、不处理数据 | 节点无需公网 IP，单向出站即可加入网络；数据原地处理，天然适应实验室防火墙环境 |

## 特性

- **🔒 数据不出域** — Agent 执行节点直接部署在实验室服务器或仪器工作站上，数据通过本地挂载原地处理，敏感数据不离开防火墙。网络只传输任务描述和结果，不触碰原始数据。
